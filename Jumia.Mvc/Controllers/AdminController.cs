    ...
    public class AdminController : Controller
    {
        ...
        public JsonResult DashBoardcount()
        {
            try
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("SELECT BrandName, COUNT(*) AS NumberOfProducts FROM dbo.Products GROUP BY BrandName", con);
                    SqlDataReader reader = cmd.ExecuteReader();
                    while (reader.Read())
                    {
                        string[] data = new string[2];
                        data[0] = reader["BrandName"].ToString();
        ...
        public JsonResult DashBoardreview()
        {
            try
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("SELECT  p.Name, AVG(r.Rating) AS AverageRating FROM  dbo.Products p JOIN reviews r ON p.Id = r.ProductID GROUP BY p.Id, p.Name ORDER BY AVG(r.Rating) DESC", con);
                    SqlDataReader reader = cmd.ExecuteReader();
                    while (reader.Read())
                    {
                        string[] data = new string[2];
        ...
        public JsonResult GetTopSellers()
        {
            ...
            try
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("SELECT TOP (3) p.Name, SUM(op.Quantity) AS TotalQuantitySold FROM [dbo].[products] p JOIN [dbo].[orderProducts] op ON p.Id = op.ProductId GROUP BY p.Id, p.Name ORDER BY TotalQuantitySold DESC", con);
                    SqlDataReader reader = cmd.ExecuteReader();
                    while (reader.Read())
                    {
